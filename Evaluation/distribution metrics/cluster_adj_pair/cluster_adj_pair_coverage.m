...
% 保存文件命名为modelName_i.mat  即part_i 与其他part连接关系的coverage结果

function cluster_adj_pair_coverage(path_t,path_g,config)
    ...
    for i=1:length(clusterNums)
        ...
        cluster_t = cluster_adj_pair_info(path_t_current,partsNum);
...
function  get_coverage_current_dir(obj1,files,partsNum)
    for i = 1:partsNum+1
       for j=i:partsNum+1
           ...
           file_current = files(TF);
           instance = struct('file','','coverage',0);
           file_name = file_current.name;
           file_path = fullfile(file_current.folder,file_name);
           data = load(file_path);
           obj = data.instance;             
           instance.file = file_current.name;
    ...
    fprintf('%s -- cluster_adj_pair_coverage -- calculate success!\n',save_path);
end
