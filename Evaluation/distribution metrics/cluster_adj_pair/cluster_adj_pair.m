%% 聚类分析part_i,part_j之间连接关系
% 使用kmeans方法对trainingSet part_i,part_j之间连接关系聚类
% 然后看生成数据落在哪些类里面，从而得到在该聚类下的分布情况

% 参数说明
% path_adj_pair--待聚类的数据路径
% path_cluster_adj_pair--聚类结果保存路径
% path_cluster_adj_pair_t --训练数据聚类结果路径
...
function cluster_adj_pair( path_adj_pair,path_cluster_adj_pair,path_cluster_adj_pair_t,config )
    ...
    for i=1:length(clusterNums)
        ...
        if strcmp(type,'T')
            % 1、当输入数据为训练数据时 根据config.clusterNums 传入的参数 使用kmeans算法将其聚成相应个数的类
            % 并将聚类结果保存
            cluster_adj_pair_t(path_adj_pair,path_cluster_adj_pair,clusterNum);
        else
            name_temp = strcat('cluster/',num2str(clusterNum));
            path_cluster_training = strrep(path_cluster_adj_pair_t,'cluster',name_temp);
            cluster_t = cluster_adj_pair_info(path_cluster_training,partsNum);
            centers = cluster_t{3};
            
            [files, ~] = get_sub_dir(path_adj_pair); 
            if(~isempty(files))
                % 2、当输入数据为生成数据时 判别生成数据落在训练数据所聚成的类中的哪一个里面，并计算其概率分布
                % 结果保存数据格式为struct(probabilities,nos,centers); 即各个类的概率 编号 聚类中心
                cluster_calculate(centers, files,clusterNum,path_cluster_adj_pair);
            end
        end
    end
    if strcmp(type,'G')
        % 3、按聚类统计得到的概率分布，计算其JSD
        % 并保存其结果，数据格式为struct(filename,jsd_value);
        cluster_adj_pair_JSD(path_cluster_adj_pair_t,path_cluster_adj_pair,config);
        % 4、按聚类情况，统计生成数据能覆盖训练数据所聚成的类中的多少个
        % 并保存其结果，数据格式为struct(filename,coverage_value);
        cluster_adj_pair_coverage(path_cluster_adj_pair_t,path_cluster_adj_pair,config);
    end
end
...
% 2、当输入数据为生成数据时 判别生成数据落在训练数据所聚成的类中的哪一个里面，并计算其概率分布
% 结果保存数据格式为struct(probabilities,nos,centers); 即各个类的概率 编号 聚类中心
function cluster_calculate(centers_trainings,files,clusterNum,path_cluster_adj_pair)
spmd
    for i_file=1:length(files)
       file_path = fullfile(files(i_file).folder,files(i_file).name);
       file = load(file_path);
       adj_generated = file.instance;
       dim = size(adj_generated,1);
       for i = 1:dim
           for j = i:dim
                centers_training = centers_trainings{i,j};
                feature_generated_cell = adj_generated(i,j);
    ...
    fprintf('%s -- cluster_adj_pair -- calculate success!\n',path_cluster_adj_pair);
end
