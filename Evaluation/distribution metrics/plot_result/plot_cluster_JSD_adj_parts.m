function plot_cluster_JSD_adj_parts( config )
...
%   此处显示详细说明
    ...
    for i=1:partsNum+1
        ...
        for idx=1:length(names)
            name = names(idx);  
            type = types(idx);
            data_i = JSD_i_cluster(paths(idx),clusterNums,i);
            if strcmp(type,'G')
                data{i,i_g,1} = data_i;
                data{i,i_g,2} = name;
                i_g = i_g + 1;
        ...
        title{i} = strcat('JSD--(',part_names(i),')');
    end
    figName = [figDir, 'adj-cluster_parts_variation'];
    view_subplot(data,data_samples,title,clusterNums, figName);
end

function matrix = JSD_i_cluster(path,clusterNums,i)
