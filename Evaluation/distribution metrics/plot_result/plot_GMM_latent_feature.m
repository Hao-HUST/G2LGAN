function plot_GMM_latent_feature( config )
%PLOT_GMM_LATENT_FEATURE 此处显示有关此函数的摘要
%   此处显示详细说明
    ...
    for idx=1:length(names)
        ...
        data_i = top_average_score(scores,top_step);
        if strcmp(type,'G') 
            data{i_g,1} = data_i;
            data{i_g,2} = name;
            i_g = i_g + 1;
    ...
    title = strcat('average-score-top-x-feature');
    figName = [figDir, 'average-score-top-x-feature'];
