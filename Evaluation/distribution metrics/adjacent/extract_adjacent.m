...
%  spmd
    for i=1:length(files)
       ...
       file = load(file_path);
       obj = file.instance;
       [matrix_pair,matrix_part] = adj_nabourhood_single(obj, parts, dims);
       count_matrix = zeros(1,parts+1);
       for ii=1:parts+1
           idx = find(obj==ii-1);
           count_matrix(1,ii) = size(idx,1);
       end
       instance= struct('count',count_matrix,'matrix_pair',matrix_pair,'matrix_part',matrix_part);
...
%     file = load(file_path);
%     data = file.instance;
%     matrix_pair = data.matrix_pair;
%     matrix_part = data.matrix_part;
%     count = data.count;
    for i=1:parts+1
       for j=1:parts+1
    ...
    for i_file=1:length(files)
        ...
        for i=1:parts+1
            if count(i)>0
                vector_pi = zeros(1,5);
                matrix = matrix_part(:,:,:,i);
                for j = 1:parts+1
                    idx = find(matrix==j);
                    vector_pi(j) = size(idx,1);
                end
                
                vector_pi = vector_pi/count(i); 
    ...
    for i=1:parts+1
        for j=i:parts+1
            temp1 = adj_statistic_pairs{i,j};
            temp2 = adj_statistic_pairs{j,i};
            instance{i,j} = [temp1,temp2];
        end
    end
